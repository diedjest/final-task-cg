...
import ru.vsu.cs.finaltaskcg.editor.ModelEditor;
import ru.vsu.cs.finaltaskcg.editor.PolygonRemover;
import ru.vsu.cs.finaltaskcg.editor.VertexRemover;
import ru.vsu.cs.finaltaskcg.objreader.ObjReader;
import ru.vsu.cs.finaltaskcg.objreader.ObjReaderException;
import ru.vsu.cs.finaltaskcg.objwriter.ObjWriter;
import ru.vsu.cs.finaltaskcg.transformations.Axis;
import ru.vsu.cs.finaltaskcg.transformations.TransformManager;
...
public class ApplicationController {
    ...
    private void deleteVertex(Model model) {
        ...
        try {
            ...
            if (index >= 0 && index < model.getVertices().size()) {
                VertexRemover.removeVertexWithDependencies(model, index);
    ...
    private void deletePolygon(Model model) {
        ...
        try {
            ...
            if (index >= 0 && index < model.getPolygons().size()) {
                PolygonRemover.removePolygonWithCleanup(model, index);
                updatePropertiesPanel(model);
                updateRenderPanel();
    ...
    private void showVertexSelectionDialog(Model model) {
        ...
        deleteButton.addActionListener(e -> {
            ...
            ModelEditor.removeSelectedVertices(model, vertexIndices);
            updatePropertiesPanel(model);
            updateRenderPanel();
            dialog.dispose();
    ...
    private void showPolygonSelectionDialog(Model model) {
        ...
        deleteButton.addActionListener(e -> {
            ...
            ModelEditor.removeSelectedPolygons(model, polygonIndices);
            updatePropertiesPanel(model);
            updateRenderPanel();
            dialog.dispose();
