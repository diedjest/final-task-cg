...
public class ApplicationController {
    ...
    @FXML
    private MenuItem openTextureMenuItem;
    ...
    private double cameraHeight = 200;
    ...
    private void setupMouseHandlers() {
        ...
        canvas.setOnMouseDragged(event -> {
            if (isMousePressed && !isMiddleMousePressed) {
                ...
                camera.pan((float)deltaX, (float)deltaY, PAN_SENSITIVITY);
                ...
                camera.rotateAroundTarget((float)deltaX, (float)deltaY, ROTATION_SENSITIVITY);
    ...
    private void render() {
        ...
        RenderEngine.setFillColor(modelColorPicker.getValue());

        RenderEngine.render(gc, camera, activeModel, (int) canvas.getWidth(), (int) canvas.getHeight());
    ...
    private void updateStats() {
        if (activeModel != null) {
            vertexCountLabel.setText("Вершин: " + activeModel.getVertices().size());
            polygonCountLabel.setText("Полигонов: " + activeModel.getPolygons().size());
        }
    }
                ...
                (observable, oldValue, newValue) -> {
                    if (newValue != null) {
                        ...
                        if (selectedIndex >= 0 && selectedIndex < models.size()) {
                            ...
                            isRendering = true;
                            statusLabel.setText("Выбрана модель: " + activeModel.getName());
                        }
                    }
                });
    ...
    private void updatePropertiesPanel(Model model) {
        if (model != null) {
            modelNameField.setText(model.getName());
        } else {
    ...
    private void loadModelFromFile() {
        ...
        if (file != null) {
            try {
                ...
                System.out.println("Загружено вершин: " + model.getVertices().size());
                System.out.println("Загружено полигонов: " + model.getPolygons().size());
                System.out.println("Загружено нормалей: " + model.getNormals().size());
                System.out.println("Загружено текстурных координат: " + model.getTextureVertices().size());
                ...
                models.add(model);
                modelListItems.add(model.getName());
                ...
                statusLabel.setText("Модель загружена: " + model.getName() +
                        " (" + model.getVertices().size() + " вершин, " +
                        model.getPolygons().size() + " полигонов)");
    ...
    private void saveModelToFile() {
        ...
        fileChooser.setInitialFileName(activeModel.getName());
        fileChooser.getExtensionFilters().add(
    ...
    private void resetTransformations() {
        if (activeModel != null) {
            showAlert("Сброс", "Функция сброса преобразований будет реализована в следующей версии");
            statusLabel.setText("Готов к работе");
        }
    }
    ...
    private void showDeleteDialog() {
        ...
        javafx.scene.control.ButtonType deleteButtonType = new javafx.scene.control.ButtonType("Удалить выбранное", javafx.scene.control.ButtonBar.ButtonData.OK_DONE);
        javafx.scene.control.ButtonType cancelButtonType = new javafx.scene.control.ButtonType("Отмена", javafx.scene.control.ButtonBar.ButtonData.CANCEL_CLOSE);
        ...
        javafx.scene.control.Tab verticesTab = new javafx.scene.control.Tab("Вершины");
        javafx.scene.control.ListView<String> verticesList = new javafx.scene.control.ListView<>();
        for (int i = 0; i < activeModel.getVertices().size(); i++) {
            verticesList.getItems().add("Вершина " + i);
        }
        verticesList.getSelectionModel().setSelectionMode(javafx.scene.control.SelectionMode.MULTIPLE);
        verticesTab.setContent(verticesList);

        javafx.scene.control.Tab polygonsTab = new javafx.scene.control.Tab("Полигоны");
        javafx.scene.control.ListView<String> polygonsList = new javafx.scene.control.ListView<>();
        for (int i = 0; i < activeModel.getPolygons().size(); i++) {
            polygonsList.getItems().add("Полигон " + i + " (" +
                    activeModel.getPolygons().get(i).getVertexIndices().size() + " вершин)");
        }
        polygonsList.getSelectionModel().setSelectionMode(javafx.scene.control.SelectionMode.MULTIPLE);
        ...
        content.getChildren().addAll(
                new javafx.scene.control.Label("Выберите элементы для удаления:"),
                tabPane,
                new javafx.scene.control.Label("Примечание: удаление вершин удалит все полигоны, содержащие эти вершины")
        );
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == deleteButtonType) {
                ...
                java.util.Set<Integer> selectedPolygons = new java.util.HashSet<>();
    ...
    private void deleteSelectedVertices(Set<Integer> verticesToRemove) {
        ArrayList<ru.vsu.cs.finaltaskcg.math.Vector3f> newVertices = new ArrayList<>();
        ArrayList<ru.vsu.cs.finaltaskcg.math.Vector3f> newNormals = new ArrayList<>();
        ArrayList<ru.vsu.cs.finaltaskcg.math.Vector2f> newTextureVertices = new ArrayList<>();

        java.util.Map<Integer, Integer> vertexMapping = new java.util.HashMap<>();
        int newIndex = 0;
        ...
        for (int i = 0; i < activeModel.getVertices().size(); i++) {
            if (!verticesToRemove.contains(i)) {
                ...
                if (i < activeModel.getNormals().size()) {
                    newNormals.add(activeModel.getNormals().get(i));
                }
                newIndex++;
            }
        }

        if (!activeModel.getTextureVertices().isEmpty()) {
            for (int i = 0; i < activeModel.getTextureVertices().size(); i++) {
                newTextureVertices.add(activeModel.getTextureVertices().get(i));
            }
        }

        ArrayList<Polygon> newPolygons = new ArrayList<>();

        for (Polygon polygon : activeModel.getPolygons()) {
        ...
        activeModel.setVertices(newVertices);
        activeModel.setNormals(newNormals);
        activeModel.setTextureVertices(newTextureVertices);
        activeModel.setPolygons(newPolygons);
    }
    ...
    private void deleteVertex(Model model) {
        ...
        dialog.setHeaderText("Введите индекс вершины");
        dialog.setContentText("Индекс (0-" + (model.getVertices().size() - 1) + "):");
        ...
        dialog.showAndWait().ifPresent(input -> {
            try {
                ...
                if (index >= 0 && index < model.getVertices().size()) {
                    isRendering = true;
                    statusLabel.setText("Вершина удалена");
                } else {
                    showAlert("Ошибка", "Некорректный индекс");
                }
            } catch (NumberFormatException e) {
    ...
    private void deletePolygon(Model model) {
        TextInputDialog dialog = new TextInputDialog("0");
        dialog.setTitle("Удаление полигона");
        dialog.setHeaderText("Введите индекс полигона");
        dialog.setContentText("Индекс (0-" + (model.getPolygons().size() - 1) + "):");
        ...
        dialog.showAndWait().ifPresent(input -> {
            try {
                ...
                if (index >= 0 && index < model.getPolygons().size()) {
                    isRendering = true;
                    statusLabel.setText("Полигон удален");
                } else {
                    showAlert("Ошибка", "Некорректный индекс");
                }
            } catch (NumberFormatException e) {
    ...
    private void showMultiSelectDialog(Model model) {
        ...
        dialog.setTitle("Множественное удаление");

        ButtonType verticesButtonType = new ButtonType("Выбрать вершины", ButtonBar.ButtonData.OK_DONE);
        ButtonType polygonsButtonType = new ButtonType("Выбрать полигоны", ButtonBar.ButtonData.OK_DONE);
        dialog.getDialogPane().getButtonTypes().addAll(verticesButtonType, polygonsButtonType, ButtonType.CANCEL);

        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == verticesButtonType) return "selectVertices";
            if (dialogButton == polygonsButtonType) return "selectPolygons";
            return null;
        });

        dialog.showAndWait().ifPresent(result -> {
            if ("selectVertices".equals(result)) {
                showVertexSelectionDialog(model);
            } else if ("selectPolygons".equals(result)) {
    ...
    private void showVertexSelectionDialog(Model model) {
        ...
        dialog.setTitle("Выбор вершин для удаления");
        ...
        for (int i = 0; i < model.getVertices().size(); i++) {
            listView.getItems().add("Вершина " + i);
        }
        listView.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);

        VBox content = new VBox(10);
        content.getChildren().addAll(new Label("Выберите вершины:"), listView);
        dialog.getDialogPane().setContent(content);
        ...
        dialog.showAndWait().ifPresent(selectedVertices -> {
            if (!selectedVertices.isEmpty()) {
                isRendering = true;
                statusLabel.setText("Удалено вершин: " + selectedVertices.size());
            }
        });
    }
    ...
    private void showPolygonSelectionDialog(Model model) {
        Dialog<Set<Integer>> dialog = new Dialog<>();
        dialog.setTitle("Выбор полигонов для удаления");
        ...
        for (int i = 0; i < model.getPolygons().size(); i++) {
            listView.getItems().add("Полигон " + i + " (вершин: " + model.getPolygons().get(i).getVertexIndices().size() + ")");
        }
        listView.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);

        VBox content = new VBox(10);
        content.getChildren().addAll(new Label("Выберите полигоны:"), listView);
        dialog.getDialogPane().setContent(content);
        ...
        dialog.showAndWait().ifPresent(selectedPolygons -> {
            if (!selectedPolygons.isEmpty()) {
                isRendering = true;
                statusLabel.setText("Удалено полигонов: " + selectedPolygons.size());
            }
        });
    }
